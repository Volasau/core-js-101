...
function getMatrixProduct(m1, m2) {
  const oneRow = m1.length;
  const oneCol = m1[0].length;
  const twoRow = m2.length;
  const twoCol = m2[0].length;
  if (oneCol !== twoRow) {
  ...
  for (let i = 0; i < oneRow; i += 1) {
    result[i] = [];
    for (let j = 0; j < twoCol; j += 1) {
      let sum = 0;
      for (let k = 0; k < oneCol; k += 1) {
        sum += m1[i][k] * m2[k][j];
      }
      result[i][j] = sum;
    }
